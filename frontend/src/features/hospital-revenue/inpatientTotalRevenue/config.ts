...
export const PAGE_HEADER_CONFIG = {
  title: "住院总收入分析",
  subtitle: "按科室与医生查看住院总收入、同比/环比以及床日趋势",
};
...
// 明细表默认排序（注意：这里是 key 字符串，和 hooks.ts 中 SortKey 对应）
export const INPATIENT_DEFAULT_SORT_KEY =
  "date" as const; // "date" | "department" | "doctor" | "revenue" ...
...
//
// 可选：一个简单的基础明细列配置（目前我们的 DetailsSection 已经自己动态拼列了，
// 如果其他地方要用到列表，可以复用这个基础配置）。
//
export const INPATIENT_DETAIL_COLUMNS_BASE: Column<DetailsRow>[] = [
  ...
  },
  {
    ...
    render: (row) => {
      const raw = row.revenue ?? (row as any).cost ?? null;
      if (raw == null) return "-";
      const n =
        typeof raw === "number"
          ? raw
