...
interface DRGCountData {
  diseaseCaseCount: number;     // DRG例数
  diseaseCoverageCount: number; // DRG覆盖病种数
}
...
interface SummaryData {
  diseaseCaseCount: number;
  diseaseCoverageCount: number;
}
...
interface DetailData {
  ...
  dep_code: string;
  dep_name: string;
  drg_case_count: number;
  drg_coverage_count: number;
...
interface OverviewResponse {
  chart: ChartDataItem[];
  summary: SummaryData;
  yoy: { [key: string]: ComparisonData };
  mom: { [key: string]: ComparisonData };
  detail: DetailData[];
}
...
// 1）初始化科室列表（保留原 init 接口）
async function fetchInitData(): Promise<InitResponse> {
  try {
    console.log('正在初始化DRG数量数据(科室列表)...');
...
// 2）聚合业务数据接口：chart + summary + yoy + mom + detail
async function fetchOverviewData(params: any): Promise<OverviewResponse> {
  try {
    console.log('正在获取DRG数量分析聚合数据 /overview:', params);
    ...
    const response = await fetch(`${API_CONFIG.baseURL}/overview?${queryParams}`, {
      method: 'GET',
      headers: API_CONFIG.headers
    });
    const data = await handleApiResponse<OverviewResponse>(response);
    console.log('聚合数据获取成功:', data);
    return data;
  } catch (error) {
...
function MultiSelect({
  ...
  return (
    <div className="w-full text左 relative">
...
function SummaryCards({
  ...
  const indicators = [
    {
      key: 'diseaseCaseCount',
      name: 'DRG例数',
      color: '#8B5CF6',
      unit: '例'
    },
    {
      key: 'diseaseCoverageCount',
      name: 'DRG覆盖病种数',
      color: '#06B6D4',
      unit: '种'
    }
        ...
        {detailData.length > 0 && (
          <div className="flex items-center gap-4 mt-2 lg:mt-0">
            ...
            <div className="text-sm text-gray-600">
              第 {safeCurrentPage} 页，共 {totalPages || 1} 页
            </div>
          </div>
        )}
      </div>
              ...
              currentData.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatNumber(item.drg_case_count)}例
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatNumber(item.drg_coverage_count)}种
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, totalPages || 1) }, (_, i) => {
                ...
                if ((totalPages || 1) <= 5) {
                  ...
                  pageNum = i + 1;
                } else if (safeCurrentPage >= (totalPages || 1) - 2) {
              ...
              {(totalPages || 1) > 5 && safeCurrentPage < (totalPages || 1) - 2 && (
                <>
...
export default function DRGCount() {
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      setError("");
      try {
        console.log('开始初始化DRG数量数据(科室 + 聚合业务)...');
        const params = getApiParams();
        const [initData, overview] = await Promise.all([
          fetchInitData(),
          fetchOverviewData(params)
        ]);

        setDepartments(initData.departments);
        setChartData(overview.chart || []);
        setSummaryData(overview.summary || null);
        setYoyData(overview.yoy || {});
        setMomData(overview.mom || {});
        setDetailData(overview.detail || []);
    ...
    initializeData();
    // 这里依赖 getApiParams，会在日期/科室默认值变更时重新触发一次
  }, [getApiParams]);
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof DRGCountData];
  ...
  const handleQuery = async (e?: React.FormEvent) => {
    ...
    try {
      const params = getApiParams();
      console.log('提交筛选查询 /overview:', params);
      const overview = await fetchOverviewData(params);
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">DRG数量分析</h1>
          </div>
