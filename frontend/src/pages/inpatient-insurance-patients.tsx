...
const YearSelector = ({
  ...
  const fetchAvailableYears = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/inpatient-insurance/years`);
...
const PeriodSelector = ({
  ...
  const fetchAvailablePeriods = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/inpatient-insurance/comparison-periods`);
...
export default function InpatientInsurancePatients() {
  ...
  const fetchData = async (range: string, year?: number) => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/inpatient-insurance?${params}`;
      console.log('请求住院医保患者数据URL:', url);

      const response = await fetch(url);

      if (!response.ok) {
        const errorText = await response.text();
        console.error('住院医保患者API响应错误:', {
      ...
      const result = await response.json();
      console.log('住院医保患者API响应:', result);

      if (result.success) {
        console.log('获取到住院医保患者数据条数:', result.data.length);
        setChartData(result.data);
      } else {
        throw new Error(result.error || 'API返回错误');
      }
    } catch (error) {
      console.error('获取住院医保患者数据失败:', error);
  ...
  const fetchSummaryData = async (range: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/inpatient-insurance/summary?range=${range}`);
  ...
  const fetchComparisonData = async (type: 'yoy' | 'mom', setData: React.Dispatch<React.SetStateAction<{ [key: string]: ComparisonData }>>) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/inpatient-insurance/comparison?type=${type}&period_date=${periodDate}`
      );
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const totalCost = Math.floor(Math.random() * 5000000) + 10000000; // 1000万-1500万
      const dischargeCount = Math.floor(Math.random() * 200) + 800; // 800-1000人次
      const avgCostPerPatient = Math.floor(Math.random() * 5000) + 15000; // 1.5万-2万
      const drugCost = Math.floor(Math.random() * 2000000) + 4000000; // 400万-600万
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (!isBackendConnected) {
        ...
        const totalCost = mockData.reduce((sum, item) => sum + item.data.totalCost, 0) / mockData.length;
        const dischargeCount = mockData.reduce((sum, item) => sum + item.data.dischargeCount, 0) / mockData.length;
        const avgCostPerPatient = mockData.reduce((sum, item) => sum + item.data.avgCostPerPatient, 0) / mockData.length;
        const drugCost = mockData.reduce((sum, item) => sum + item.data.drugCost, 0) / mockData.length;
  ...
  const getSummaryValue = (key: keyof InpatientInsuranceData) => {
    ...
    return value !== null && value !== undefined
      ? `${Math.round(value).toLocaleString()}${indicator?.unit || ''}`
      : `0${indicator?.unit || ''}`;
  };
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `住院医保患者情况趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof InpatientInsuranceData];
  ...
  const formatValue = (num: any, unit: string) => {
    ...
    if (unit === '元' && numberValue >= 10000) {
      return `${(numberValue / 10000).toFixed(2)}万${unit}`;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">医保住院患者情况</h2>
        ...
        {indicators.map((indicator) => (
          <div key={indicator.key} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-3">
              {getSummaryValue(indicator.key as keyof InpatientInsuranceData)}
            </div>
        ...
        <div className="flex items-start">
          ...
          <div className="text-green-800 text-sm">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              ...
              <li><strong>住院医保患者总费用</strong> = 期内出院患者医保总费用之和</li>
              <li><strong>医保患者出院人次</strong> = 期内医保患者出院人次之和</li>
              <li><strong>住院医保患者次均费用</strong> = 期内出院医保患者医药费用 / 同期医保患者出院人数</li>
              <li><strong>住院医保患者药费</strong> = 期内出院医保患者药品费用之和</li>
