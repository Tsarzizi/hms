...
// ==================== API 配置 ====================
const API_CONFIG = {
  baseURL: '/api/inpatient-avg-cost-ratio',
  timeout: 30000,
  headers: {
...
function DetailedDataTable({
  ...
  // 获取科室显示文本
  const getDepartmentDisplayText = useCallback((departmentId: string) => {
  ...
  const getFilterSummary = useCallback(() => {
    ...
    const selectedDepartmentNames = Array.from(selectedDeps)
      .map(depId => {
    ...
    if (selectedDepartmentNames.length === 1) {
      return selectedDepartmentNames[0];
    }

    // 如果选择的科室数量较多，显示前2个科室+等
    if (selectedDepartmentNames.length > 2) {
      return `${selectedDepartmentNames.slice(0, 2).join('、')}等${selectedDepartmentNames.length}个科室`;
    }

    return selectedDepartmentNames.join('、');
              ...
              currentData.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                    {getDepartmentDisplayText(item.departmentId)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.data.drugCostRatio.toFixed(1)}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.data.materialCostRatio.toFixed(1)}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.data.examinationCostRatio.toFixed(1)}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.data.treatmentCostRatio.toFixed(1)}%
                  </td>
                </tr>
              ))
            ) : (
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: labels.map(date => {
          ...
          // 计算平均值或使用第一个科室的数据
          const total = items.reduce((sum, item) => sum + item.data[indicator.key as keyof CostRatioData], 0);
