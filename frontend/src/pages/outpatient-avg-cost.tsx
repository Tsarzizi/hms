...
export default function OutpatientAvgCost() {
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const totalAvgCost = Math.floor(Math.random() * 100) + 200;
      const outpatientAvgCost = Math.floor(Math.random() * 80) + 180;
      const emergencyAvgCost = Math.floor(Math.random() * 150) + 250;
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatValue(item.data[indicator.key as keyof OutpatientCostData], indicator.unit)}
                      </td>
                    ))}
                  </tr>
                ))
