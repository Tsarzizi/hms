...
interface SummaryData {
  outpatientEmergencyAvgCost: number;
  outpatientAvgCost: number;
  emergencyAvgCost: number;
  outpatientEmergencyDrugCost: number;
}
...
const YearSelector = ({
  ...
  const fetchAvailableYears = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/insurance-patient-cost/years`);
...
const PeriodSelector = ({
  ...
  const fetchAvailablePeriods = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/insurance-patient-cost/comparison-periods`);
...
export default function InsurancePatientCost() {
  ...
  const fetchData = async (range: string, year?: number) => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/insurance-patient-cost?${params}`;
      console.log('请求医保患者费用URL:', url);
      ...
      const result = await response.json();
      console.log('医保患者费用API响应:', result);

      if (result.success) {
        console.log('获取到医保患者费用数据条数:', result.data.length);
        setChartData(result.data);
      } else {
        throw new Error(result.error || 'API返回错误');
      }
    } catch (error) {
      console.error('获取医保患者费用数据失败:', error);
      // 不再设置错误状态，直接使用模拟数据
  ...
  const fetchSummaryData = async (range: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/insurance-patient-cost/summary?range=${range}`);
  ...
  const fetchComparisonData = async (type: 'yoy' | 'mom', setData: React.Dispatch<React.SetStateAction<{ [key: string]: ComparisonData }>>) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/insurance-patient-cost/comparison?type=${type}&period_date=${periodDate}`
      );
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const outpatientEmergencyAvgCost = Math.floor(Math.random() * 200) + 300; // 300-500元
      const outpatientAvgCost = Math.floor(Math.random() * 150) + 250; // 250-400元
      const emergencyAvgCost = Math.floor(Math.random() * 300) + 400; // 400-700元
      const outpatientEmergencyDrugCost = Math.floor(Math.random() * 100) + 150; // 150-250元
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (!isBackendConnected) {
        ...
        // 计算摘要数据
        const avgOutpatientEmergency = mockData.reduce((sum, item) => sum + item.data.outpatientEmergencyAvgCost, 0) / mockData.length;
        const avgOutpatient = mockData.reduce((sum, item) => sum + item.data.outpatientAvgCost, 0) / mockData.length;
        const avgEmergency = mockData.reduce((sum, item) => sum + item.data.emergencyAvgCost, 0) / mockData.length;
        const avgDrugCost = mockData.reduce((sum, item) => sum + item.data.outpatientEmergencyDrugCost, 0) / mockData.length;

        setSummaryData({
          outpatientEmergencyAvgCost: avgOutpatientEmergency,
          outpatientAvgCost: avgOutpatient,
          emergencyAvgCost: avgEmergency,
          outpatientEmergencyDrugCost: avgDrugCost
        });
        return;
      }
  ...
  const getSummaryValue = (key: keyof InsurancePatientCostData) => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof InsurancePatientCostData];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">医保患者费用</h2>
        ...
        {indicators.map((indicator) => (
          <div key={indicator.key} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-3">
              {getSummaryValue(indicator.key as keyof InsurancePatientCostData)}
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {indicator.key === 'outpatientEmergencyAvgCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>门急诊医保患者次均费用（元）=期内门急诊医保基金支付之和/同期门急诊医保患者就诊人次
                </p>
              )}
              {indicator.key === 'outpatientAvgCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>门诊医保患者次均费用（元）=期内门诊医保基金支付之和/同期门诊医保患者就诊人次
                </p>
              )}
              {indicator.key === 'emergencyAvgCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>急诊医保患者次均费用（元）=期内急诊医保基金支付之和/同期急诊医保患者就诊人次
                </p>
              )}
              {indicator.key === 'outpatientEmergencyDrugCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>门急诊医保患者药费（元）=期内门诊和急诊医保患者的药品费用之和
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex items-start">
          ...
          <div className="text-green-800 text-sm">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              ...
              <li><strong>门急诊医保患者次均费用</strong> = 期内门急诊医保基金支付之和/同期门急诊医保患者就诊人次</li>
              <li><strong>门诊医保患者次均费用</strong> = 期内门诊医保基金支付之和/同期门诊医保患者就诊人次</li>
              <li><strong>急诊医保患者次均费用</strong> = 期内急诊医保基金支付之和/同期急诊医保患者就诊人次</li>
              <li><strong>门急诊医保患者药费</strong> = 期内门诊和急诊医保患者的药品费用之和</li>
