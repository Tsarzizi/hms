...
interface RevenueStructureData {
  drugCostStructure: number; // 门诊患者医药费用构成(%)
  departmentRevenueStructure: number; // 门诊各科室收入构成(%)
  regionalRevenueStructure: number; // 门诊按地域来源收入构成(%)
}
...
const indicators = [
  {
    key: 'drugCostStructure',
    name: '门诊患者医药费用构成',
    color: '#3B82F6',
    description: '各项目费用构成占门诊收入的比重',
    formula: '各项目费用构成(%) = 各项目费用/门诊收入×100%',
    unit: '%'
  },
  {
    key: 'departmentRevenueStructure',
    name: '门诊各科室收入构成',
    color: '#10B981',
    description: '各科室收入占门诊收入的比重',
    formula: '科室收入构成(%) = 某科室收入/门诊收入×100%',
    unit: '%'
  },
  {
    key: 'regionalRevenueStructure',
    name: '门诊按地域来源收入构成',
    color: '#EF4444',
    description: '各地区患者门诊收入占门诊总收入的比重',
    formula: '患者地域来源收入构成(%) = 各地患者门诊收入/门诊总收入×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            drugCostStructure: parseFloat((35 + Math.sin(i * 0.3) * 8 + Math.random() * 4).toFixed(1)),
            departmentRevenueStructure: parseFloat((40 + Math.cos(i * 0.2) * 10 + Math.random() * 5).toFixed(1)),
            regionalRevenueStructure: parseFloat((25 + Math.sin(i * 0.4) * 6 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            drugCostStructure: parseFloat((37 + Math.sin(i * 0.5) * 6 + Math.random() * 3).toFixed(1)),
            departmentRevenueStructure: parseFloat((42 + Math.cos(i * 0.3) * 8 + Math.random() * 4).toFixed(1)),
            regionalRevenueStructure: parseFloat((28 + Math.sin(i * 0.6) * 5 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            drugCostStructure: parseFloat((39 + Math.sin(i * 0.7) * 4 + Math.random() * 2).toFixed(1)),
            departmentRevenueStructure: parseFloat((44 + Math.cos(i * 0.4) * 6 + Math.random() * 3).toFixed(1)),
            regionalRevenueStructure: parseFloat((30 + Math.sin(i * 0.8) * 4 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            drugCostStructure: parseFloat((40 + Math.sin(i * 0.6) * 3 + Math.random() * 1).toFixed(1)),
            departmentRevenueStructure: parseFloat((45 + Math.cos(i * 0.5) * 4 + Math.random() * 2).toFixed(1)),
            regionalRevenueStructure: parseFloat((32 + Math.sin(i * 1) * 3 + Math.random() * 1).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof RevenueStructureData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    drugCostStructure: {
      value: lastData.drugCostStructure,
      change: prevData ? (lastData.drugCostStructure - prevData.drugCostStructure) : 0
    },
    departmentRevenueStructure: {
      value: lastData.departmentRevenueStructure,
      change: prevData ? (lastData.departmentRevenueStructure - prevData.departmentRevenueStructure) : 0
    },
    regionalRevenueStructure: {
      value: lastData.regionalRevenueStructure,
      change: prevData ? (lastData.regionalRevenueStructure - prevData.regionalRevenueStructure) : 0
    }
  };
};
...
export default function RevenueStructure() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `门急诊收入结构指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof RevenueStructureData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">门急诊收入结构分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析门急诊收入结构的各项指标，包括患者医药费用构成、各科室收入构成、地域来源收入构成等关键数据，支持同比环比分析
            </p>
          </div>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof RevenueStructureData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>门诊患者医药费用构成：各项目费用构成(%) = 各项目费用/门诊收入×100%</li>
              <li>门诊各科室收入构成：科室收入构成(%) = 某科室收入/门诊收入×100%</li>
              <li>门诊按地域来源收入构成：患者地域来源收入构成(%) = 各地患者门诊收入/门诊总收入×100%</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解收入结构的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>收入结构分析有助于了解医院收入的组成和分布情况</li>
              <li>医药费用构成分析可帮助了解药品、检查、治疗等各项费用的占比</li>
              <li>科室收入构成分析可识别各科室对医院总收入的贡献程度</li>
              <li>地域来源收入构成分析有助于了解医院服务覆盖范围和区域影响力</li>
            </ul>
          </div>
        </div>
      </section>
