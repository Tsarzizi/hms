...
interface SpecialClinicData {
  specialClinicServiceRatio: number; // 特需门诊服务人次数比例(%)
}
...
const indicators = [
  {
    key: 'specialClinicServiceRatio',
    name: '特需门诊服务人次数比例',
    color: '#8B5CF6',
    description: '特需门诊服务人次数比例',
    formula: '特需门诊服务人次数比例(%)=特需门诊服务人次/门诊人次×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            specialClinicServiceRatio: parseFloat((8 + Math.sin(i * 0.3) * 3 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            specialClinicServiceRatio: parseFloat((9 + Math.sin(i * 0.5) * 2 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            specialClinicServiceRatio: parseFloat((10 + Math.sin(i * 0.7) * 1.5 + Math.random() * 1).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            specialClinicServiceRatio: parseFloat((11 + Math.sin(i * 0.6) * 1 + Math.random() * 0.5).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof SpecialClinicData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    specialClinicServiceRatio: {
      value: lastData.specialClinicServiceRatio,
      change: prevData ? (lastData.specialClinicServiceRatio - prevData.specialClinicServiceRatio) : 0
    }
  };
};

// 计算平均服务比例
const calculateAverageRatio = (data: ChartData[]): number => {
  if (data.length === 0) return 0;
  const total = data.reduce((sum, item) => sum + item.data.specialClinicServiceRatio, 0);
  return total / data.length;
};

// 获取最新数据
const getLatestData = (data: ChartData[]): number => {
  if (data.length === 0) return 0;
  const latest = data[data.length - 1];
  return latest.data.specialClinicServiceRatio;
};

export default function SpecialClinic() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `特需门诊指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof SpecialClinicData]),
  ...
  const averageRatio = calculateAverageRatio(chartData);
    ...
    <div className="min-h-screen bg-gray-50 p-6 space-y-6">
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">特需门诊指标分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析特需门诊服务的各项指标，包括特需门诊服务人次数比例等关键数据，支持同比环比分析
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">平均服务比例</h3>
            <div
              className="w-3 h-3 rounded-full flex-shrink-0"
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm">
              <span className="text-gray-400 text-sm">当前时间范围平均值</span>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 font-medium mb-2">统计说明：</p>
              <p className="text-xs text-gray-500 leading-relaxed">
                当前时间范围内特需门诊服务人次数比例的平均值
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">最新数据</h3>
            <div
              className="w-3 h-3 rounded-full flex-shrink-0"
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm">
              <span className="text-gray-400 text-sm">当前最新统计值</span>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 font-medium mb-2">统计说明：</p>
              <p className="text-xs text-gray-500 leading-relaxed">
                当前时间范围内最新的特需门诊服务比例数据
              </p>
            </div>
          </div>
        </div>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof SpecialClinicData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>特需门诊服务人次数比例反映特需医疗服务在整体门诊服务中的占比</li>
              <li>计算公式：特需门诊服务人次 ÷ 门诊总人次 × 100%</li>
              <li>该指标有助于评估医院特需医疗服务的发展水平和市场需求</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解特需门诊服务的发展趋势和季节性特征</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>特需门诊服务是医院提供的高端医疗服务，通常包括专家门诊、VIP服务等</li>
              <li>特需门诊服务比例的提升通常反映医院服务质量的提升和品牌影响力的增强</li>
              <li>合理的特需门诊比例有助于优化医院收入结构，提高医疗服务价值</li>
            </ul>
          </div>
        </div>
      </section>
