...
export default function InsurancePatientTotalCost() {
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const outpatientEmergencyCost = Math.floor(Math.random() * 200) + 300; // 300-500万元
      const outpatientCost = Math.floor(Math.random() * 150) + 200; // 200-350万元
      const emergencyCost = Math.floor(Math.random() * 100) + 50; // 50-150万元
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">医保患者总费用</h2>
        ...
        {indicators.map((indicator) => (
          <div key={indicator.key} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {indicator.key === 'outpatientEmergencyCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>门急诊医保患者总费用（万元）=期内门急诊患者医保范围内总额之和
                </p>
              )}
              {indicator.key === 'outpatientCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>门诊医保患者总费用（万元）=期内门诊患者医保范围内总额之和
                </p>
              )}
              {indicator.key === 'emergencyCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>急诊医保患者总费用（万元）=期内急诊患者医保范围内总额之和
                </p>
              )}
            </div>
          </div>
