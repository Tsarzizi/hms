...
const indicators = [
  { 
    ...
    formula: '体检收入环比增长率(%) = (本期体检收入-上期体检收入)/上期体检收入×100%',
    unit: '%'
  },
  { 
    ...
    formula: '体检收入同比增长率(%) = (本期体检收入-上期同期体检收入)/上期同期体检收入×100%',
    unit: '%'
  }
];
...
export default function PhysicalExaminationRevenueGrowthRate() {
  ...
  const fetchData = async (range: string) => {
    ...
    try {
      // 这里应该调用实际的后端API
      // const response = await fetch(`/api/physical-exam-revenue-growth-rate?range=${range}`);
  ...
  const fetchComparisonData = async () => {
    try {
      // 这里应该调用实际的后端API
      // const yoyResponse = await fetch(`/api/physical-exam-revenue-growth-rate/comparison?type=yoy&range=${timeRange}`);
      // const momResponse = await fetch(`/api/physical-exam-revenue-growth-rate/comparison?type=mom&range=${timeRange}`);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">体检收入增长率分析</h2>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 lg:mb-0">体检收入增长率趋势分析图表</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">体检收入环比增长率(%)</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">体检收入同比增长率(%)</th>
              </tr>
            </thead>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start">
          ...
          <div className="text-blue-800 text-sm">
            <p className="font-medium mb-1">体检收入增长率指标说明：</p>
            <ul className="list-disc list-inside space-y-1 text-xs">
              <li><strong>体检收入环比增长率：</strong>(本期体检收入-上期体检收入)/上期体检收入×100%</li>
              <li><strong>体检收入同比增长率：</strong>(本期体检收入-上期同期体检收入)/上期同期体检收入×100%</li>
              <li>数据来源于医院信息系统，每日更新</li>
              <li>增长率指标反映体检收入变化趋势和发展速度</li>
