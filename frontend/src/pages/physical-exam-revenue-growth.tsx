...
interface RevenueGrowthRateData {
  physicalExamRevenueMomGrowthRate: number; // 体检收入环比增长率(%)
  physicalExamRevenueYoyGrowthRate: number; // 体检收入同比增长率(%)
}

interface ChartData {
  date: string;
  data: RevenueGrowthRateData;
}
...
const indicators = [
  {
    key: 'physicalExamRevenueMomGrowthRate',
    name: '体检收入环比增长率',
    color: '#3B82F6',
    description: '体检收入环比增长率(%)=(本期体检收入-上期体检收入)/上期体检收入×100%',
    unit: '%'
  },
  {
    key: 'physicalExamRevenueYoyGrowthRate',
    name: '体检收入同比增长率',
    color: '#10B981',
    description: '体检收入同比增长率(%)=(本期体检收入-上期同期体检收入)/上期同期体检收入×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            physicalExamRevenueMomGrowthRate: parseFloat((Math.sin(i * 0.3) * 8 + Math.random() * 4).toFixed(1)),
            physicalExamRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.2) * 6 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            physicalExamRevenueMomGrowthRate: parseFloat((Math.sin(i * 0.5) * 6 + Math.random() * 3).toFixed(1)),
            physicalExamRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.4) * 5 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            physicalExamRevenueMomGrowthRate: parseFloat((Math.sin(i * 0.7) * 5 + Math.random() * 2).toFixed(1)),
            physicalExamRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.6) * 4 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            physicalExamRevenueMomGrowthRate: parseFloat((Math.sin(i * 0.6) * 4 + Math.random() * 1).toFixed(1)),
            physicalExamRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.5) * 3 + Math.random() * 0.8).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof RevenueGrowthRateData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    physicalExamRevenueMomGrowthRate: {
      value: lastData.physicalExamRevenueMomGrowthRate,
      change: prevData ? (lastData.physicalExamRevenueMomGrowthRate - prevData.physicalExamRevenueMomGrowthRate) : 0
    },
    physicalExamRevenueYoyGrowthRate: {
      value: lastData.physicalExamRevenueYoyGrowthRate,
      change: prevData ? (lastData.physicalExamRevenueYoyGrowthRate - prevData.physicalExamRevenueYoyGrowthRate) : 0
    }
  };
};

export default function PhysicalExaminationRevenueGrowthRate() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `体检收入增长率趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof RevenueGrowthRateData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">体检收入增长率分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析体检收入增长率的各项指标，包括体检收入环比增长率、体检收入同比增长率等关键数据，支持同比环比分析
            </p>
          </div>
                    ...
                    {comparison ? (
                      <>
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          {comparison.current > 0 ? '+' : ''}{comparison.current.toFixed(1)}% vs {comparison.previous > 0 ? '+' : ''}{comparison.previous.toFixed(1)}%
                        </div>
                      </>
                    ) : (
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof RevenueGrowthRateData] > 0 ? '+' : ''}{item.data[indicator.key as keyof RevenueGrowthRateData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>体检收入环比增长率反映短期内收入变化趋势</li>
              <li>体检收入同比增长率反映长期收入发展状况</li>
              <li>计算公式：环比增长率=(本期收入-上期收入)/上期收入×100%</li>
              <li>计算公式：同比增长率=(本期收入-去年同期收入)/去年同期收入×100%</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解体检收入增长的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>增长率为正值表示增长，负值表示下降</li>
            </ul>
          </div>
        </div>
      </section>
