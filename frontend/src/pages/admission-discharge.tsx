  ...
  switch (range) {
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const baseAdmission = 120 + Math.sin(i * 0.3) * 30 + Math.random() * 20;
        const baseDischarge = 110 + Math.cos(i * 0.3) * 25 + Math.random() * 15;

        data.push({
          date: dateStr,
          data: {
            admissionCount: Math.round(baseAdmission),
            dischargeCount: Math.round(baseDischarge),
            inpatientRatio: parseFloat((85 + Math.sin(i * 0.2) * 10 + Math.random() * 5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const baseAdmission = 3500 + Math.sin(i * 0.5) * 800 + Math.random() * 400;
        const baseDischarge = 3400 + Math.cos(i * 0.5) * 700 + Math.random() * 300;
        ...
        data.push({
          ...
          data: {
            admissionCount: Math.round(baseAdmission),
            dischargeCount: Math.round(baseDischarge),
            inpatientRatio: parseFloat((88 + Math.sin(i * 0.3) * 8 + Math.random() * 4).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        const baseAdmission = 10500 + Math.sin(i * 0.7) * 2500 + Math.random() * 1000;
        const baseDischarge = 10200 + Math.cos(i * 0.7) * 2200 + Math.random() * 800;
        ...
        data.push({
          ...
          data: {
            admissionCount: Math.round(baseAdmission),
            dischargeCount: Math.round(baseDischarge),
            inpatientRatio: parseFloat((90 + Math.sin(i * 0.4) * 6 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const baseAdmission = 42000 + Math.sin(i) * 6000 + Math.random() * 3000;
        const baseDischarge = 40800 + Math.cos(i) * 5500 + Math.random() * 2500;
        ...
        data.push({
          ...
          data: {
            admissionCount: Math.round(baseAdmission),
            dischargeCount: Math.round(baseDischarge),
            inpatientRatio: parseFloat((92 + Math.sin(i * 0.6) * 4 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof AdmissionDischargeData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    admissionCount: {
      value: lastData.admissionCount,
      change: prevData ? ((lastData.admissionCount - prevData.admissionCount) / prevData.admissionCount * 100) : 0
    },
    dischargeCount: {
      value: lastData.dischargeCount,
      change: prevData ? ((lastData.dischargeCount - prevData.dischargeCount) / prevData.dischargeCount * 100) : 0
    },
    inpatientRatio: {
      value: lastData.inpatientRatio,
      change: prevData ? (lastData.inpatientRatio - prevData.inpatientRatio) : 0
    }
  };
};
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        tension: 0.1,
        yAxisID: indicator.key === 'inpatientRatio' ? 'y1' : 'y'
      }));
        ...
        <div className="flex items-center justify-between">
          <div className="text-left">
            ...
            <p className="text-gray-600 text-sm mt-2">
              监控和分析住院服务的出入院人次指标，包括入院人次、出院人次、住院人头人次比等核心数据，支持同比环比分析
            </p>
          </div>
                    ...
                    {comparison ? (
                      <>
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          {formatNumber(comparison.current)} vs {formatNumber(comparison.previous)}
                        </div>
                      </>
                    ) : (
                ...
                chartData.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.admissionCount.toLocaleString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.dischargeCount.toLocaleString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.inpatientRatio}%
                    </td>
                  </tr>
                ))
              ) : (
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>同比分析显示与去年同期相比的增减情况</li>
              <li>环比分析显示与上期相比的增减情况</li>
              <li>支持按科室、医生等维度进行深入分析</li>
            </ul>
          </div>
        </div>
      </section>
