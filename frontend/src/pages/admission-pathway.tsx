...
const mockDoctors = [
  ...
  { value: "doctor_6", label: "杨医生" },
  { value: "doctor_7", label: "赵医生" },
  { value: "doctor_8", label: "黄医生" }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            emergencyAdmissionRate: parseFloat((25 + Math.sin(i * 0.3) * 8 + Math.random() * 4).toFixed(1)),
            outpatientAdmissionRate: parseFloat((15 + Math.cos(i * 0.3) * 5 + Math.random() * 3).toFixed(1)),
            transferAdmissionRate: parseFloat((8 + Math.sin(i * 0.2) * 3 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            emergencyAdmissionRate: parseFloat((26 + Math.sin(i * 0.5) * 6 + Math.random() * 3).toFixed(1)),
            outpatientAdmissionRate: parseFloat((16 + Math.cos(i * 0.5) * 4 + Math.random() * 2).toFixed(1)),
            transferAdmissionRate: parseFloat((9 + Math.sin(i * 0.3) * 2 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            emergencyAdmissionRate: parseFloat((27 + Math.sin(i * 0.7) * 5 + Math.random() * 2).toFixed(1)),
            outpatientAdmissionRate: parseFloat((17 + Math.cos(i * 0.7) * 3 + Math.random() * 1.5).toFixed(1)),
            transferAdmissionRate: parseFloat((10 + Math.sin(i * 0.4) * 1.5 + Math.random() * 1).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            emergencyAdmissionRate: parseFloat((28 + Math.sin(i) * 4 + Math.random() * 1.5).toFixed(1)),
            outpatientAdmissionRate: parseFloat((18 + Math.cos(i) * 2.5 + Math.random() * 1).toFixed(1)),
            transferAdmissionRate: parseFloat((11 + Math.sin(i * 0.6) * 1 + Math.random() * 0.5).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof AdmissionPathwayData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    emergencyAdmissionRate: {
      ...
      change: prevData ? ((lastData.emergencyAdmissionRate - prevData.emergencyAdmissionRate) / prevData.emergencyAdmissionRate * 100) : 0
    },
    outpatientAdmissionRate: {
      value: lastData.outpatientAdmissionRate,
      change: prevData ? ((lastData.outpatientAdmissionRate - prevData.outpatientAdmissionRate) / prevData.outpatientAdmissionRate * 100) : 0
    },
    transferAdmissionRate: {
      value: lastData.transferAdmissionRate,
      change: prevData ? ((lastData.transferAdmissionRate - prevData.transferAdmissionRate) / prevData.transferAdmissionRate * 100) : 0
    }
  };
};
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">入院途径指标分析</h1>
                ...
                chartData.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.emergencyAdmissionRate.toFixed(1)}%
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.outpatientAdmissionRate.toFixed(1)}%
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.data.transferAdmissionRate.toFixed(1)}%
                    </td>
                  </tr>
                ))
              ) : (
      ...
      <section className="bg-blue-50 border border-blue-200 rounded-xl p-6">
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            <h3 className="font-medium mb-2 text-lg">指标说明：</h3>
