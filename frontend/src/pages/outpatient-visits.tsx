...
interface OutpatientData {
  ...
  outpatientPerCapitaRatio: number;
}
...
const indicators = [
  {
    ...
    name: '总诊疗人次数',
    color: '#3B82F6',
    description: '报告期内某地区所有诊疗活动的总人次，包括医疗卫生机构的门诊、急诊、出诊、单项健康检查、健康咨询指导人次',
    unit: '人次'
  },
  {
    ...
    name: '门急诊人次',
    color: '#EF4444',
    description: '报告期内某地区医疗卫生机构的门诊和急诊人次数之和',
    unit: '人次'
  },
  {
    ...
    name: '门诊人次',
    color: '#10B981',
    description: '报告期内某地区医疗卫生机构的门诊的人次数之和',
    unit: '人次'
  },
  {
    ...
    name: '门诊人次增减率',
    color: '#F59E0B',
    description: '门诊人次增减率=（本期门诊人次-上期门诊人次）/上期门诊人次×100%',
    unit: '%'
  },
  {
    key: 'emergencyVisits',
    name: '急诊人次',
    color: '#8B5CF6',
    description: '报告期内某地区医疗卫生机构的急诊的人次数之和',
    unit: '人次'
  },
  {
    ...
    name: '急诊人次增减率',
    color: '#EC4899',
    description: '急诊人次增减率=（本期急诊人次-上期急诊人次）/上期急诊人次×100%',
    unit: '%'
  },
  {
    key: 'outpatientPerCapitaRatio',
    name: '门诊人头人次比',
    color: '#06B6D4',
    description: '门诊人头人次比=门诊患者人数/门诊患者人次数×100%',
    unit: '%'
  }
];
  ...
  switch (range) {
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const totalVisits = 50000 + Math.sin(i * 0.5) * 8000 + Math.random() * 3000;
        const outpatientVisits = 35000 + Math.sin(i * 0.4) * 5000 + Math.random() * 2000;
        const emergencyVisits = 15000 + Math.sin(i * 0.6) * 3000 + Math.random() * 1000;
        const outpatientEmergencyVisits = outpatientVisits + emergencyVisits;

        data.push({
          date: dateStr,
          data: {
            totalVisits: parseFloat(totalVisits.toFixed(0)),
            outpatientEmergencyVisits: parseFloat(outpatientEmergencyVisits.toFixed(0)),
            outpatientVisits: parseFloat(outpatientVisits.toFixed(0)),
            outpatientGrowthRate: parseFloat((Math.random() * 10 - 5).toFixed(2)),
            emergencyVisits: parseFloat(emergencyVisits.toFixed(0)),
            emergencyGrowthRate: parseFloat((Math.random() * 8 - 4).toFixed(2)),
            outpatientPerCapitaRatio: parseFloat((1.2 + Math.random() * 0.3).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        const totalVisits = 52000 + Math.sin(i * 0.7) * 6000 + Math.random() * 2500;
        const outpatientVisits = 36000 + Math.sin(i * 0.5) * 4000 + Math.random() * 1500;
        const emergencyVisits = 16000 + Math.sin(i * 0.8) * 2000 + Math.random() * 800;
        const outpatientEmergencyVisits = outpatientVisits + emergencyVisits;
        ...
        data.push({
          ...
          data: {
            ...
            outpatientEmergencyVisits: parseFloat(outpatientEmergencyVisits.toFixed(0)),
            outpatientVisits: parseFloat(outpatientVisits.toFixed(0)),
            outpatientGrowthRate: parseFloat((Math.random() * 8 - 4).toFixed(2)),
            emergencyVisits: parseFloat(emergencyVisits.toFixed(0)),
            emergencyGrowthRate: parseFloat((Math.random() * 6 - 3).toFixed(2)),
            outpatientPerCapitaRatio: parseFloat((1.3 + Math.random() * 0.2).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const totalVisits = 55000 + Math.sin(i * 0.6) * 5000 + Math.random() * 2000;
        const outpatientVisits = 38000 + Math.sin(i * 0.7) * 3000 + Math.random() * 1000;
        const emergencyVisits = 17000 + Math.sin(i * 1.0) * 1500 + Math.random() * 500;
        const outpatientEmergencyVisits = outpatientVisits + emergencyVisits;
        ...
        data.push({
          ...
          data: {
            ...
            outpatientEmergencyVisits: parseFloat(outpatientEmergencyVisits.toFixed(0)),
            outpatientVisits: parseFloat(outpatientVisits.toFixed(0)),
            outpatientGrowthRate: parseFloat((Math.random() * 6 - 3).toFixed(2)),
            emergencyVisits: parseFloat(emergencyVisits.toFixed(0)),
            emergencyGrowthRate: parseFloat((Math.random() * 4 - 2).toFixed(2)),
            outpatientPerCapitaRatio: parseFloat((1.4 + Math.random() * 0.1).toFixed(2))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof OutpatientData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    totalVisits: {
      value: lastData.totalVisits,
      change: prevData ? (lastData.totalVisits - prevData.totalVisits) : 0
    },
    outpatientEmergencyVisits: {
      value: lastData.outpatientEmergencyVisits,
      change: prevData ? (lastData.outpatientEmergencyVisits - prevData.outpatientEmergencyVisits) : 0
    },
    outpatientVisits: {
      value: lastData.outpatientVisits,
      change: prevData ? (lastData.outpatientVisits - prevData.outpatientVisits) : 0
    },
    outpatientGrowthRate: {
      value: lastData.outpatientGrowthRate,
      change: prevData ? (lastData.outpatientGrowthRate - prevData.outpatientGrowthRate) : 0
    },
    emergencyVisits: {
      value: lastData.emergencyVisits,
      change: prevData ? (lastData.emergencyVisits - prevData.emergencyVisits) : 0
    },
    emergencyGrowthRate: {
      value: lastData.emergencyGrowthRate,
      change: prevData ? (lastData.emergencyGrowthRate - prevData.emergencyGrowthRate) : 0
    },
    outpatientPerCapitaRatio: {
      value: lastData.outpatientPerCapitaRatio,
      change: prevData ? (lastData.outpatientPerCapitaRatio - prevData.outpatientPerCapitaRatio) : 0
    }
  };
};
...
export default function OutpatientVisits() {
  ...
  const [selectedIndicators, setSelectedIndicators] = useState<string[]>(
    ['totalVisits', 'outpatientVisits', 'emergencyVisits']
  );
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `门急诊人次指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof OutpatientData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">门急诊人次指标分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析门急诊服务的各项人次指标，包括总诊疗人次、门急诊人次、增减率等关键数据，支持同比环比分析
            </p>
          </div>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {indicator.unit === '%' ?
                          `${item.data[indicator.key as keyof OutpatientData] >= 0 ? '+' : ''}${item.data[indicator.key as keyof OutpatientData].toFixed(2)}${indicator.unit}` :
                          `${formatNumber(item.data[indicator.key as keyof OutpatientData])}${indicator.unit}`
                        }
                      </td>
                    ))}
                  </tr>
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              <li>数据来源于医院门急诊系统，每日更新</li>
              <li><strong>总诊疗人次数</strong>：报告期内某地区所有诊疗活动的总人次，包括医疗卫生机构的门诊、急诊、出诊、单项健康检查、健康咨询指导人次</li>
              <li><strong>门急诊人次</strong>：报告期内某地区医疗卫生机构的门诊和急诊人次数之和</li>
              <li><strong>门诊人次</strong>：报告期内某地区医疗卫生机构的门诊的人次数之和</li>
              <li><strong>门诊人次增减率</strong> =（本期门诊人次-上期门诊人次）/上期门诊人次×100%</li>
              <li><strong>急诊人次</strong>：报告期内某地区医疗卫生机构的急诊的人次数之和</li>
              <li><strong>急诊人次增减率</strong> =（本期急诊人次-上期急诊人次）/上期急诊人次×100%</li>
              <li><strong>门诊人头人次比</strong> = 门诊患者人数/门诊患者人次数×100%</li>
              <li>支持按月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解门急诊服务的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>后端服务运行在: {API_BASE_URL || '默认端口'}</li>
            </ul>
          </div>
        </div>
      </section>
