...
interface FollowUpData {
  followUpRate: number; // 复诊率(%)
  followUpAppointmentRate: number; // 复诊预约率(%)
}
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            followUpRate: parseFloat((35 + Math.sin(i * 0.3) * 8 + Math.random() * 4).toFixed(1)),
            followUpAppointmentRate: parseFloat((65 + Math.sin(i * 0.4) * 10 + Math.random() * 5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            followUpRate: parseFloat((38 + Math.sin(i * 0.5) * 6 + Math.random() * 3).toFixed(1)),
            followUpAppointmentRate: parseFloat((68 + Math.sin(i * 0.6) * 8 + Math.random() * 4).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            followUpRate: parseFloat((40 + Math.sin(i * 0.7) * 5 + Math.random() * 2).toFixed(1)),
            followUpAppointmentRate: parseFloat((70 + Math.sin(i * 0.8) * 6 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            followUpRate: parseFloat((42 + Math.sin(i * 0.6) * 4 + Math.random() * 2).toFixed(1)),
            followUpAppointmentRate: parseFloat((72 + Math.sin(i * 0.7) * 5 + Math.random() * 2.5).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof FollowUpData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    followUpRate: {
      value: lastData.followUpRate,
      change: prevData ? (lastData.followUpRate - prevData.followUpRate) : 0
    },
    followUpAppointmentRate: {
      value: lastData.followUpAppointmentRate,
      change: prevData ? (lastData.followUpAppointmentRate - prevData.followUpAppointmentRate) : 0
    }
  };
};
...
export default function FollowUpService() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `复诊情况指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof FollowUpData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">复诊情况指标分析</h1>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof FollowUpData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
