...
interface DRGCostData {
  avgCost: number;           // DRG出院患者平均费用
  drugCostRatio: number;     // DRG药品费用占比
  materialCostRatio: number; // DRG卫生材料费用占比
}
          ...
          <div key={indicator.key} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="text-3xl font-bold text-gray-900">
                {stat ? (
                  <>
                    {indicator.key === 'avgCost' ? formatCurrency(stat) : `${stat.toFixed(2)}${indicator.unit}`}
                  </>
                ) : (
            ...
            {currentData.length > 0 ? (
              currentData.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.drug_cost_ratio.toFixed(2)}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {item.material_cost_ratio.toFixed(2)}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
...
export default function DRGCost() {
  ...
  const getChartOptions = () => ({
    ...
    scales: {
      ...
      y: {
        title: {
          display: true,
          text: selectedIndicators.includes('avgCost') ? '费用 (元) / 百分比 (%)' : '百分比 (%)'
        },
        beginAtZero: true,
        ticks: {
          callback: function(value: any) {
            if ((selectedIndicators.includes('drugCostRatio') || selectedIndicators.includes('materialCostRatio')) && typeof value === 'number') {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof DRGCostData];
        ...
        fill: false,
        showLine: true,
        yAxisID: (indicator.key === 'drugCostRatio' || indicator.key === 'materialCostRatio') ? 'y1' : 'y'
      }));
    ...
    return {
      ...
      datasets,
      // 为百分比指标添加第二个Y轴
      ...((selectedIndicators.includes('drugCostRatio') || selectedIndicators.includes('materialCostRatio')) && {
        scales: {
          y: {
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">DRG费用分析</h1>
          </div>
