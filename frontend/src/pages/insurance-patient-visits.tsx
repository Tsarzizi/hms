...
export default function InsurancePatientVisits() {
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const outpatientEmergencyVisits = Math.floor(Math.random() * 5000) + 10000;
      const outpatientVisits = Math.floor(Math.random() * 4000) + 8000;
      const emergencyVisits = Math.floor(Math.random() * 1000) + 2000;
      const outpatientEmergencyPatients = Math.floor(Math.random() * 3000) + 6000;
      const revisitRate = Math.floor(Math.random() * 20) + 30;
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">医保患者就诊情况分析</h1>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatValue(item.data[indicator.key as keyof InsuranceVisitData], indicator.unit)}
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>医保就诊分析有助于了解医院医保患者就诊模式和复诊情况</li>
