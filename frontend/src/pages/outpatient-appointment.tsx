...
async function fetchSummaryAPI(params: ApiParams): Promise<any> {
  try {
    console.log('正在获取门急诊人次汇总数据:', params);
...
async function fetchDetailsAPI(params: ApiParams): Promise<{ rows: DetailsRow[]; total: number }> {
  try {
    console.log('正在获取门急诊人次明细数据:', params);
...
async function fetchTimeseriesAPI(params: ApiParams): Promise<{ rows: TSRow[] }> {
  try {
    console.log('正在获取门急诊人次趋势数据:', params);
...
function DetailsTable({
  ...
  total: number;
  loading: boolean;
  sortKey: "date" | "doctorName" | "department" | "outpatientVisits" | "emergencyVisits";
  sortDir: 'asc' | 'desc';
  onChangeSortKey: (key: "date" | "doctorName" | "department" | "outpatientVisits" | "emergencyVisits") => void;
  ...
  const handleSort = (key: "date" | "doctorName" | "department" | "outpatientVisits" | "emergencyVisits") => {
    if (sortKey === key) {
  ...
  const SortIcon = ({ columnKey }: { columnKey: "date" | "doctorName" | "department" | "outpatientVisits" | "emergencyVisits" }) => {
...
function TrendSection({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-900">门急诊人次指标趋势分析</h3>
      </div>
...
export default function OutpatientVisits() {
  ...
  const [sortKey, setSortKey] = useState<"date" | "doctorName" | "department" | "outpatientVisits" | "emergencyVisits">("date");
        ...
        <div className="flex items-center justify-between">
          <div className="text-left">
            ...
            <p className="text-gray-600 text-sm mt-2">
              监控和分析门急诊服务的各项人次指标，包括门急诊总人次、门诊人次、增减率等关键数据，支持同比环比分析
            </p>
          </div>
