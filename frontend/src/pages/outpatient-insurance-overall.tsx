    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            avgMedicalCost: parseFloat((8000 + Math.sin(i * 0.5) * 1000 + Math.random() * 500).toFixed(2)),
            avgDrugCost: parseFloat((3000 + Math.sin(i * 0.3) * 500 + Math.random() * 200).toFixed(2)),
            avgDailyMedicalCost: parseFloat((800 + Math.sin(i * 0.4) * 100 + Math.random() * 50).toFixed(2)),
            trendData: parseFloat((8000 + Math.sin(i * 0.6) * 800 + Math.random() * 400).toFixed(2)),
            costBySource: parseFloat((7000 + Math.sin(i * 0.7) * 800 + Math.random() * 300).toFixed(2)),
            costByDisease: parseFloat((6000 + Math.sin(i * 0.8) * 700 + Math.random() * 350).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            avgMedicalCost: parseFloat((8200 + Math.sin(i * 0.7) * 800 + Math.random() * 400).toFixed(2)),
            avgDrugCost: parseFloat((3200 + Math.sin(i * 0.5) * 400 + Math.random() * 150).toFixed(2)),
            avgDailyMedicalCost: parseFloat((850 + Math.sin(i * 0.6) * 80 + Math.random() * 40).toFixed(2)),
            trendData: parseFloat((8200 + Math.sin(i * 0.8) * 600 + Math.random() * 300).toFixed(2)),
            costBySource: parseFloat((7200 + Math.sin(i * 0.9) * 600 + Math.random() * 250).toFixed(2)),
            costByDisease: parseFloat((6200 + Math.sin(i * 1.0) * 500 + Math.random() * 280).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            avgMedicalCost: parseFloat((8500 + Math.sin(i * 0.6) * 600 + Math.random() * 300).toFixed(2)),
            avgDrugCost: parseFloat((3500 + Math.sin(i * 0.4) * 300 + Math.random() * 100).toFixed(2)),
            avgDailyMedicalCost: parseFloat((900 + Math.sin(i * 0.8) * 60 + Math.random() * 30).toFixed(2)),
            trendData: parseFloat((8500 + Math.sin(i * 1.0) * 400 + Math.random() * 200).toFixed(2)),
            costBySource: parseFloat((7500 + Math.sin(i * 1.1) * 400 + Math.random() * 200).toFixed(2)),
            costByDisease: parseFloat((6500 + Math.sin(i * 1.2) * 300 + Math.random() * 200).toFixed(2))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof InpatientCostData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    ...
    trendData: {
      value: lastData.trendData,
      change: prevData ? (lastData.trendData - prevData.trendData) : 0
    },
    costBySource: {
      value: lastData.costBySource,
      change: prevData ? (lastData.costBySource - prevData.costBySource) : 0
    },
    costByDisease: {
      value: lastData.costByDisease,
      change: prevData ? (lastData.costByDisease - prevData.costByDisease) : 0
    }
  };
};
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof InpatientCostData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">住院患者次均医药费用</h1>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof InpatientCostData].toFixed(2)}{indicator.unit}
                      </td>
                    ))}
                  </tr>
                ))
