...
interface InpatientCostData {
  avgMedicalCost: number;          // 住院患者次均医药费用
  avgDrugCost: number;             // 住院患者次均药费
  avgDailyMedicalCost: number;     // 住院患者日均医药费用
  trendData: number;               // 住院患者次均变化趋势
  costBySource: number;            // 不同来源患者住院次均费用
  costByDisease: number;           // 各病种住院患者次均费用
}

interface ChartData {
  date: string;
  data: InpatientCostData;
}
...
const indicators = [
  {
    ...
    name: '住院患者次均医药费用',
    color: '#3B82F6',
    description: '住院患者次均医药费用=报告期内出院者住院医药费用/同期出院人数',
    unit: '元'
  },
  {
    key: 'avgDrugCost',
    name: '住院患者次均药费',
    color: '#10B981',
    description: '住院患者次均药费用=报告期内出院者住院药费/同期出院人数',
    unit: '元'
  },
  {
    ...
    name: '住院患者日均医药费用',
    color: '#F59E0B',
    description: '住院患者日均医药费用=报告期内出院者医药费用总额/同期出院者住院天数',
    unit: '元'
  },
  {
    key: 'trendData',
  ...
  },
  {
    ...
    name: '各病种住院患者次均费用',
    color: '#06B6D4',
    description: '某病种住院患者次均医药费用=报告期内该病种出院者住院医药总费用/报告期内该病种出院患者总人数',
    unit: '元'
  }
];
...
const YearSelector = ({
  ...
  const fetchAvailableYears = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/inpatient-cost/years`);
...
const PeriodSelector = ({
  ...
  const fetchAvailablePeriods = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/inpatient-cost/comparison-periods`);
...
export default function InpatientMedicalCost() {
  ...
  const fetchData = async (range: string, year?: number) => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/inpatient-cost?${params}`;
      console.log('请求住院费用URL:', url);
      ...
      const result = await response.json();
      console.log('住院费用API响应:', result);

      if (result.success) {
        console.log('获取到住院费用数据条数:', result.data.length);
        setChartData(result.data);
      } else {
        throw new Error(result.error || 'API返回错误');
      }
    } catch (error) {
      console.error('获取住院费用数据失败:', error);
      // 不再设置错误状态，直接使用模拟数据
  ...
  const fetchSummaryData = async (range: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/inpatient-cost/summary?range=${range}`);
  ...
  const fetchComparisonData = async (type: 'yoy' | 'mom', setData: React.Dispatch<React.SetStateAction<{ [key: string]: ComparisonData }>>) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/inpatient-cost/comparison?type=${type}&period_date=${periodDate}`
      );
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const avgMedicalCost = Math.floor(Math.random() * 5000) + 8000; // 8000-13000元
      const avgDrugCost = Math.floor(Math.random() * 2000) + 3000; // 3000-5000元
      const avgDailyMedicalCost = Math.floor(Math.random() * 500) + 800; // 800-1300元
      const trendData = avgMedicalCost; // 与次均费用相同
      const costBySource = Math.floor(Math.random() * 3000) + 7000; // 7000-10000元
      const costByDisease = Math.floor(Math.random() * 4000) + 6000; // 6000-10000元
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (!isBackendConnected) {
        ...
        // 计算摘要数据
        const totalAvgMedicalCost = mockData.reduce((sum, item) => sum + item.data.avgMedicalCost, 0) / mockData.length;
        const totalAvgDrugCost = mockData.reduce((sum, item) => sum + item.data.avgDrugCost, 0) / mockData.length;
        const totalAvgDailyMedicalCost = mockData.reduce((sum, item) => sum + item.data.avgDailyMedicalCost, 0) / mockData.length;
        const totalTrendData = mockData.reduce((sum, item) => sum + item.data.trendData, 0) / mockData.length;
        const totalCostBySource = mockData.reduce((sum, item) => sum + item.data.costBySource, 0) / mockData.length;
        const totalCostByDisease = mockData.reduce((sum, item) => sum + item.data.costByDisease, 0) / mockData.length;

        setSummaryData({
          avgMedicalCost: totalAvgMedicalCost,
          avgDrugCost: totalAvgDrugCost,
          avgDailyMedicalCost: totalAvgDailyMedicalCost,
          trendData: totalTrendData,
          costBySource: totalCostBySource,
          costByDisease: totalCostByDisease
        });
        return;
      }
  ...
  const getSummaryValue = (key: keyof InpatientCostData) => {
    if (!summaryData) return '暂无数据';
    const value = summaryData[key];
    const indicator = indicators.find(ind => ind.key === key);

    if (key === 'avgMedicalCost' || key === 'avgDrugCost' || key === 'avgDailyMedicalCost' ||
        key === 'trendData' || key === 'costBySource' || key === 'costByDisease') {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `住院医疗费用指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof InpatientCostData];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">住院患者次均医药费用</h2>
        <p className="text-gray-600 text-sm">
          监控和分析医院住院患者医疗费用情况，包括次均医药费用、次均药费、日均医药费用等关键指标，支持同比环比分析
        </p>
      </div>
        ...
        {indicators.map((indicator) => (
          <div key={indicator.key} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mb-3">
              {getSummaryValue(indicator.key as keyof InpatientCostData)}
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {indicator.key === 'avgMedicalCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>住院患者次均医药费用 = 报告期内出院者住院医药费用 / 同期出院人数
                </p>
              )}
              {indicator.key === 'avgDrugCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>住院患者次均药费用 = 报告期内出院者住院药费 / 同期出院人数
                </p>
              )}
              {indicator.key === 'avgDailyMedicalCost' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>住院患者日均医药费用 = 报告期内出院者医药费用总额 / 同期出院者住院天数
                </p>
              )}
              {indicator.key === 'trendData' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>住院患者次均医药费用 = 报告期内出院者住院医药费用 / 同期出院人数
                </p>
              )}
              {indicator.key === 'costBySource' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>住院患者次均医药费用 = 报告期内出院者住院医药费用 / 同期出院人数
                </p>
              )}
              {indicator.key === 'costByDisease' && (
                <p className="text-xs text-gray-500 mt-2">
                  <strong>计算公式：</strong>某病种住院患者次均医药费用 = 报告期内该病种出院者住院医药总费用 / 报告期内该病种出院患者总人数
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex items-start">
          ...
          <div className="text-green-800 text-sm">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              ...
              <li><strong>不同来源患者住院次均费用</strong> = 报告期内出院者住院医药费用 / 同期出院人数</li>
              <li><strong>各病种住院患者次均费用</strong> = 报告期内该病种出院者住院医药总费用 / 报告期内该病种出院患者总人数</li>
              <li>住院费用管理是医院成本控制的重要组成部分，反映医院医疗费用控制水平</li>
