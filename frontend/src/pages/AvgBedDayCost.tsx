...
async function fetchChartData(params: any): Promise<ChartDataItem[]> {
  try {
    ...
    const response = await fetch(`${API_CONFIG.baseURL}/bed-day-cost/chart?${queryParams}`, {
      method: 'GET',
      headers: API_CONFIG.headers
    });
          ...
          <div key={indicator.key} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="text-3xl font-bold text-gray-900">
                {stat ? (
                  <>
                    {indicator.key === 'overallAvgBedDayCost' ? formatCurrency(stat) : formatNumber(stat)}
              ...
              currentData.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatCurrency(item.avg_bed_day_cost)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => {
          const value = item.data[indicator.key as keyof SummaryData];
