...
// ==================== API 配置 ====================
const API_CONFIG = {
  baseURL: '/api/outpatient-avg-cost',
  timeout: 30000,
  headers: {
...
async function fetchChartData(params: any): Promise<ChartData[]> {
  try {
    ...
    const data = await handleApiResponse<ChartData[]>(response);
...
interface DetailData {
  billing_date: string;    // 时间
  dep_code: string;        // 科室代码
  dep_name: string;        // 科室名称
  costs: number;           // 费用
  visit_count: number;     // 数量
}
...
export default function OutpatientAvgCost() {
  ...
  const [detailData, setDetailData] = useState<DetailData[]>([]); // 详细数据状态
  ...
  // 获取详细数据的API函数
  const fetchDetailData = useCallback(async (params: any): Promise<DetailData[]> => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.baseURL}/detail?${queryParams}`, {
        method: 'GET',
        headers: API_CONFIG.headers
      });
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        const [chart, summary, yoy, mom, detail] = await Promise.all([
          fetchChartData(params),
          fetchSummaryData(params),
          fetchComparisonData('yoy', params),
          fetchComparisonData('mom', params),
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 space-y-6">
      {/* 页面标题 */}
      <header className="bg白 rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">门急诊次均费用分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析医院门急诊次均费用情况，包括门急诊总体次均费用、门诊患者次均费用、急诊患者次均费用等关键指标
            </p>
          </div>
      ...
      {/* 图表区域 */}
      <section className="bg白 rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
      ...
      {/* 详细数据表格 + 分页 */}
      <section className="bg白 rounded-xl.shadow-sm border border-gray-200 p-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {detailData.length > 0 ? (
                pagedDetailData.map((item, index) => (
        ...
        {detailData.length > 0 && (
          <div className="flex items-center justify-between mt-4 text-sm text-gray-700">
            ...
            <div>
              第 {currentPage} 页 / 共 {Math.max(1, Math.ceil(detailData.length / pageSize))} 页
            </div>
            <button
              type="button"
              onClick={() =>
                setCurrentPage((p) =>
                  p >= Math.ceil(detailData.length / pageSize) ? p : p + 1
                )
              }
              disabled={currentPage >= Math.ceil(detailData.length / pageSize)}
              className="px-3 py-1.5 border border-gray-300 rounded-md bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              下一页
            </button>
