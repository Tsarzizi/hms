...
interface HealthExaminationData {
  healthExaminationCount: number; // 健康检查人次
  healthExaminationAvgCost: number; // 健康检查次均费用
}

interface ChartData {
  date: string;
  data: HealthExaminationData;
}
...
const indicators = [
  {
    key: 'healthExaminationCount',
    name: '健康检查人次',
    color: '#3B82F6',
    description: '报告期内某地区医疗机构全身体检人次数和体检中心、全身及单项健康检查人次数之和（人次和项目不要重复统计）',
    unit: '人次'
  },
  {
    key: 'healthExaminationAvgCost',
    name: '健康检查次均费用',
    color: '#EF4444',
    description: '健康检查次均费用=每月健康检查总费用/健康检查人次数',
    unit: '元'
  }
];
  ...
  switch (range) {
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const healthExaminationCount = 500 + Math.sin(i * 0.5) * 100 + Math.random() * 50;
        const healthExaminationAvgCost = 300 + Math.sin(i * 0.3) * 50 + Math.random() * 25;

        data.push({
          date: dateStr,
          data: {
            healthExaminationCount: parseFloat(healthExaminationCount.toFixed(0)),
            healthExaminationAvgCost: parseFloat(healthExaminationAvgCost.toFixed(2))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        const healthExaminationCount = 550 + Math.sin(i * 0.7) * 80 + Math.random() * 40;
        const healthExaminationAvgCost = 320 + Math.sin(i * 0.5) * 40 + Math.random() * 20;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const healthExaminationCount = 600 + Math.sin(i * 0.6) * 60 + Math.random() * 30;
        const healthExaminationAvgCost = 350 + Math.sin(i * 0.8) * 30 + Math.random() * 15;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof HealthExaminationData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    healthExaminationCount: {
      value: lastData.healthExaminationCount,
      change: prevData ? (lastData.healthExaminationCount - prevData.healthExaminationCount) : 0
    },
    healthExaminationAvgCost: {
      value: lastData.healthExaminationAvgCost,
      change: prevData ? (lastData.healthExaminationAvgCost - prevData.healthExaminationAvgCost) : 0
    }
  };
};

// 计算平均值
const calculateAverages = (data: ChartData[]) => {
  if (data.length === 0) return null;

  const totalCount = data.reduce((sum, item) => sum + item.data.healthExaminationCount, 0);
  const totalCost = data.reduce((sum, item) => sum + item.data.healthExaminationAvgCost, 0);

  return {
    avgCount: totalCount / data.length,
...
export default function HealthExamination() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `体检人次指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof HealthExaminationData]),
    ...
    <div className="min-h-screen bg-gray-50 p-6 space-y-6">
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">体检人次指标分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析体检服务的各项指标，包括健康检查人次、健康检查次均费用等关键数据，支持同比环比分析
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">平均健康检查人次</h3>
            <div
              className="w-3 h-3 rounded-full"
          ...
          <div className="space-y-3">
            <div className="text-3xl font-bold text-gray-900">
              {averages ? (
                <>
                  {formatNumber(averages.avgCount)}
                  <span className="text-lg font-normal ml-1 text-gray-500">人次</span>
                </>
              ) : (
                '暂无数据'
              )}
            ...
            <div className="text-sm">
              <span className="text-gray-400 text-sm">统计期间平均值</span>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 font-medium mb-2">统计说明：</p>
              <p className="text-xs text-gray-500 leading-relaxed">
                当前时间范围内健康检查人次的平均值，反映体检服务的平均规模
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">平均检查费用</h3>
            <div
              className="w-3 h-3 rounded-full"
          ...
          <div className="space-y-3">
            <div className="text-3xl font-bold text-gray-900">
              {averages ? (
                <>
                  {averages.avgCost.toFixed(2)}
                  <span className="text-lg font-normal ml-1 text-gray-500">元</span>
                </>
              ) : (
                '暂无数据'
              )}
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 font-medium mb-2">统计说明：</p>
              <p className="text-xs text-gray-500 leading-relaxed">
                当前时间范围内健康检查次均费用的平均值，反映体检服务的平均成本水平
              </p>
            </div>
          </div>
        </div>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {indicator.unit === '元' ?
                          `${item.data[indicator.key as keyof HealthExaminationData].toFixed(2)}${indicator.unit}` :
                          `${formatNumber(item.data[indicator.key as keyof HealthExaminationData])}${indicator.unit}`
                        }
                      </td>
                    ))}
                  </tr>
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              <li>数据来源于医院信息系统和体检中心系统，每日更新</li>
              <li><strong>健康检查人次</strong>：报告期内某地区医疗机构全身体检人次数和体检中心、全身及单项健康检查人次数之和（人次和项目不要重复统计）</li>
              <li><strong>健康检查次均费用</strong> = 每月健康检查总费用 / 健康检查人次数</li>
              <li>健康检查人次反映体检服务的规模和需求</li>
              <li>健康检查次均费用反映体检服务的成本效益</li>
              <li>支持按月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解体检服务的发展趋势和季节性变化</li>
