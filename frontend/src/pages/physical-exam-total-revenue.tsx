...
interface PhysicalExaminationRevenueData {
  personalExamRevenue: number; // 个人体检收入(万元)
}

interface ChartData {
  date: string;
  data: PhysicalExaminationRevenueData;
}
...
const indicators = [
  { 
    key: 'personalExamRevenue', 
    name: '个人体检收入', 
    color: '#3B82F6', 
    description: '挂号类别代码为体检的总费用汇总求和',
    unit: '万元'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenue: parseFloat((120 + Math.sin(i * 0.3) * 40 + Math.random() * 20).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenue: parseFloat((150 + Math.sin(i * 0.5) * 50 + Math.random() * 30).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenue: parseFloat((180 + Math.sin(i * 0.7) * 60 + Math.random() * 40).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenue: parseFloat((200 + Math.sin(i * 0.6) * 70 + Math.random() * 50).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof PhysicalExaminationRevenueData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    personalExamRevenue: {
      value: lastData.personalExamRevenue,
      change: prevData ? (lastData.personalExamRevenue - prevData.personalExamRevenue) : 0
    }
  };
};

export default function PhysicalExaminationRevenue() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `体检总收入趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof PhysicalExaminationRevenueData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">体检总收入分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析体检收入的各项指标，包括个人体检收入等关键数据，支持同比环比分析
            </p>
          </div>
                ...
                <div className="text-sm">
                  {stat && stat.change !== 0 ? (
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {stat.change > 0 ? '↑' : '↓'} {Math.abs(stat.change).toFixed(1)}{indicator.unit}
                    ...
                    {comparison ? (
                      <>
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          {formatCurrency(comparison.current)}{indicator.unit} vs {formatCurrency(comparison.previous)}{indicator.unit}
                        </div>
                      </>
                    ) : (
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatCurrency(item.data[indicator.key as keyof PhysicalExaminationRevenueData])}{indicator.unit}
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>个人体检收入反映体检业务的总体收入规模</li>
              <li>计算公式：挂号类别代码为体检的总费用汇总求和</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解收入的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>所有金额数据均以人民币万元为单位显示</li>
              <li>收入增长趋势有助于评估体检业务的经营状况和发展潜力</li>
            </ul>
          </div>
        </div>
      </section>
