...
interface RevenueGrowthRateData {
  ...
  outpatientEmergencyRevenueYoyGrowthRate: number; // 门急诊收入同比增长率(%)
}
...
const indicators = [
  {
    ...
    name: '门诊患者医药费用增长率',
    color: '#3B82F6',
    description: '门诊患者医药费用增长率(%)=(报告期门诊患者医药费用-上期门诊患者医药费用)/上期门诊患者医药费用×100%',
    unit: '%'
  },
  {
    key: 'outpatientEmergencyRevenueYoyGrowthRate',
    name: '门急诊收入同比增长率',
    color: '#10B981',
    description: '门诊收入同比增长率(%)=(本期门诊收入-去年同期门诊收入)/上期门诊收入×100%；急诊收入同比增长率(%)=(本期急诊收入-去年同期急诊收入)/上期急诊收入×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            outpatientMedicalCostGrowthRate: parseFloat((Math.sin(i * 0.3) * 6 + Math.random() * 3).toFixed(1)),
            outpatientEmergencyRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.2) * 5 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            outpatientMedicalCostGrowthRate: parseFloat((Math.sin(i * 0.5) * 5 + Math.random() * 2).toFixed(1)),
            outpatientEmergencyRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.4) * 4 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            outpatientMedicalCostGrowthRate: parseFloat((Math.sin(i * 0.7) * 4 + Math.random() * 1.5).toFixed(1)),
            outpatientEmergencyRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.6) * 3 + Math.random() * 1).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            outpatientMedicalCostGrowthRate: parseFloat((Math.sin(i * 0.6) * 3 + Math.random() * 1).toFixed(1)),
            outpatientEmergencyRevenueYoyGrowthRate: parseFloat((Math.cos(i * 0.5) * 2 + Math.random() * 0.8).toFixed(1))
          }
        });
      }
      break;
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    outpatientMedicalCostGrowthRate: {
      value: lastData.outpatientMedicalCostGrowthRate,
      change: prevData ? (lastData.outpatientMedicalCostGrowthRate - prevData.outpatientMedicalCostGrowthRate) : 0
    },
    outpatientEmergencyRevenueYoyGrowthRate: {
      value: lastData.outpatientEmergencyRevenueYoyGrowthRate,
      change: prevData ? (lastData.outpatientEmergencyRevenueYoyGrowthRate - prevData.outpatientEmergencyRevenueYoyGrowthRate) : 0
    }
  };
};
...
export default function RevenueGrowthRate() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `门急诊收入增长率趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
        ...
        <div className="flex items-center justify-between">
          <div className="text-left">
            ...
            <p className="text-gray-600 text-sm mt-2">
              监控和分析门急诊收入增长率的各项指标，包括门诊患者医药费用增长率、门急诊收入同比增长率等关键数据，支持同比环比分析
            </p>
          </div>
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>门诊患者医药费用增长率反映患者医疗费用变化趋势</li>
              <li>门急诊收入同比增长率反映医院长期收入发展状况</li>
              <li>计算公式：门诊患者医药费用增长率=(报告期门诊患者医药费用-上期门诊患者医药费用)/上期门诊患者医药费用×100%</li>
              <li>计算公式：门急诊收入同比增长率=(本期门急诊收入-去年同期门急诊收入)/去年同期门急诊收入×100%</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解收入增长的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>增长率为正值表示增长，负值表示下降</li>
              <li>合理控制医药费用增长率有助于减轻患者负担</li>
            </ul>
          </div>
        </div>
      </section>
