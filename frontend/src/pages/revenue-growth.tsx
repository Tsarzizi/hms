...
const indicators = [
  { 
    ...
    formula: '门诊患者医药费用增长率(%) = (报告期门诊患者医药费用-上期门诊患者医药费用)/上期门诊患者医药费用×100%',
    unit: '%'
  },
  { 
    ...
    formula: '门诊收入同比增长率(%) = (本期门诊收入-去年同期门诊收入)/上期门诊收入×100%；急诊收入同比增长率(%) = (本期急诊收入-去年同期急诊收入)/上期急诊收入×100%',
    unit: '%'
  }
];
...
export default function RevenueGrowthRate() {
  ...
  const fetchData = async (range: string) => {
    ...
    try {
      // 这里应该调用实际的后端API
      // const response = await fetch(`/api/revenue-growth-rate?range=${range}`);
  ...
  const fetchComparisonData = async () => {
    try {
      // 这里应该调用实际的后端API
      // const yoyResponse = await fetch(`/api/revenue-growth-rate/comparison?type=yoy&range=${timeRange}`);
      // const momResponse = await fetch(`/api/revenue-growth-rate/comparison?type=mom&range=${timeRange}`);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2">门急诊收入增长率分析</h2>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 lg:mb-0">收入增长率趋势分析图表</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">门诊患者医药费用增长率(%)</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">门急诊收入同比增长率(%)</th>
              </tr>
            </thead>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start">
          ...
          <div className="text-blue-800 text-sm">
            <p className="font-medium mb-1">门急诊收入增长率指标说明：</p>
            <ul className="list-disc list-inside space-y-1 text-xs">
              <li><strong>门诊患者医药费用增长率：</strong>(报告期门诊患者医药费用-上期门诊患者医药费用)/上期门诊患者医药费用×100%</li>
              <li><strong>门急诊收入同比增长率：</strong>门诊收入同比增长率=(本期门诊收入-去年同期门诊收入)/上期门诊收入×100%；急诊收入同比增长率=(本期急诊收入-去年同期急诊收入)/上期急诊收入×100%</li>
              <li>数据来源于医院信息系统，每日更新</li>
              <li>增长率指标反映收入变化趋势和发展速度</li>
