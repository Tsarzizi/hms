    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            medicalInsuranceAverageRevenue: parseFloat((450 + Math.sin(i * 0.3) * 80 + Math.random() * 40).toFixed(2)),
            nonMedicalInsuranceAverageRevenue: parseFloat((680 + Math.cos(i * 0.3) * 120 + Math.random() * 60).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            medicalInsuranceAverageRevenue: parseFloat((480 + Math.sin(i * 0.5) * 60 + Math.random() * 30).toFixed(2)),
            nonMedicalInsuranceAverageRevenue: parseFloat((720 + Math.cos(i * 0.5) * 100 + Math.random() * 50).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            medicalInsuranceAverageRevenue: parseFloat((500 + Math.sin(i * 0.7) * 50 + Math.random() * 25).toFixed(2)),
            nonMedicalInsuranceAverageRevenue: parseFloat((750 + Math.cos(i * 0.7) * 80 + Math.random() * 40).toFixed(2))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            medicalInsuranceAverageRevenue: parseFloat((520 + Math.sin(i) * 40 + Math.random() * 20).toFixed(2)),
            nonMedicalInsuranceAverageRevenue: parseFloat((780 + Math.cos(i) * 60 + Math.random() * 30).toFixed(2))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof AverageRevenueData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    medicalInsuranceAverageRevenue: {
      value: lastData.medicalInsuranceAverageRevenue,
      change: prevData ? ((lastData.medicalInsuranceAverageRevenue - prevData.medicalInsuranceAverageRevenue) / prevData.medicalInsuranceAverageRevenue * 100) : 0
    },
    nonMedicalInsuranceAverageRevenue: {
      value: lastData.nonMedicalInsuranceAverageRevenue,
      change: prevData ? ((lastData.nonMedicalInsuranceAverageRevenue - prevData.nonMedicalInsuranceAverageRevenue) / prevData.nonMedicalInsuranceAverageRevenue * 100) : 0
    }
  };
};
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">门急诊次均收入分析</h1>
