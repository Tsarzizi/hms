...
export default function DoctorWorkloadPerformance() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        // 移除默认选中设置，使用空Set
        setSelectedDeps(new Set<string>());
        setSelectedDoctors(new Set<string>());
  ...
  useEffect(() => {
    const updateCurrentUserInfo = () => {
      if (selectedDoctors.size > 0) {
        ...
        setCurrentUserInfo((prev) => ({ ...prev, name: "全部医生" }));
      }
  ...
  const handleQuery = async () => {
    // 修改：移除必须选择筛选条件的限制，允许空筛选条件查询全部数据
    setLoading(true);
    setError("");
    try {
  ...
  const handleExportExcel = async () => {
    // 修改：移除必须选择筛选条件的限制，允许空筛选条件导出全部数据
    try {
      setExporting(true);
      const params = new URLSearchParams();
      ...
      <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="w-full border border-gray-300 rounded-lg px-3 py-2.5 bg-gray-50">
              <span className="text-gray-900">{currentUserInfo.department}</span>
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 block">
            ...
            <div className="w-full border border-gray-300 rounded-lg px-3 py-2.5 bg-gray-50">
              <span className="text-gray-900">{currentUserInfo.name}</span>
            </div>
          </div>
        </div>
      </section>

      {/* 绩效板块 - 总是显示，当没有选择筛选条件时显示全部数据 */}
