...
export default function OutpatientAvgDrugCost() {
  ...
  const getMockData = () => {
    ...
    months.forEach(month => {
      const totalAvgDrugCost = Math.floor(Math.random() * 50) + 80;
      const outpatientAvgDrugCost = Math.floor(Math.random() * 40) + 70;
      const emergencyAvgDrugCost = Math.floor(Math.random() * 60) + 90;
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatValue(item.data[indicator.key as keyof DrugCostData], indicator.unit)}
                      </td>
                    ))}
                  </tr>
                ))
