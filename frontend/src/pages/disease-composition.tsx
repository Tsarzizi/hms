...
interface DiseaseCompositionData {
  diseaseCompositionRate: number; // 出院患者疾病构成(%)
}
...
const indicators = [
  { 
    key: 'diseaseCompositionRate', 
    name: '出院患者疾病构成', 
    color: '#3B82F6', 
    description: '某种疾病出院患者人次/出院患者人次×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            diseaseCompositionRate: parseFloat((25 + Math.sin(i * 0.3) * 10 + Math.random() * 5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            diseaseCompositionRate: parseFloat((28 + Math.sin(i * 0.5) * 8 + Math.random() * 4).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            diseaseCompositionRate: parseFloat((30 + Math.sin(i * 0.7) * 6 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            diseaseCompositionRate: parseFloat((32 + Math.sin(i * 0.6) * 4 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof DiseaseCompositionData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    diseaseCompositionRate: {
      value: lastData.diseaseCompositionRate,
      change: prevData ? (lastData.diseaseCompositionRate - prevData.diseaseCompositionRate) : 0
    }
  };
};

export default function DiseaseComposition() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `疾病构成指标趋势图 (${timeRanges.find(r => r.key === timeRange)?.label})`
      },
    },
    scales: {
      x: {
  ...
  const getChartData = () => {
    ...
    const datasets = indicators
      ...
      .map(indicator => ({
        ...
        data: chartData.map(item => item.data[indicator.key as keyof DiseaseCompositionData]),
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">疾病构成指标分析</h1>
            <p className="text-gray-600 text-sm mt-2">
              监控和分析出院患者疾病构成情况，包括各种疾病在出院患者中的占比分布，支持同比环比分析
            </p>
          </div>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof DiseaseCompositionData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>出院患者疾病构成反映各种疾病在出院患者中的分布情况</li>
              <li>计算公式：某种疾病出院患者人次/出院患者人次×100%</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解疾病构成的变化趋势和季节性特征</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>疾病构成分析有助于了解医院重点疾病和治疗方向</li>
            </ul>
          </div>
        </div>
      </section>
