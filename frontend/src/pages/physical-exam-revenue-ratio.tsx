...
const indicators = [
  {
    ...
    name: '个人体检收入占比',
    color: '#3B82F6',
    description: '个人体检收入占比(%) = 个人体检收入/门急诊收入(含体检收入)×100%',
    unit: '%'
  }
];
    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenueRatio: parseFloat((15 + Math.sin(i * 0.3) * 5 + Math.random() * 3).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenueRatio: parseFloat((18 + Math.sin(i * 0.5) * 4 + Math.random() * 2).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenueRatio: parseFloat((20 + Math.sin(i * 0.7) * 3 + Math.random() * 1.5).toFixed(1))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            personalExamRevenueRatio: parseFloat((22 + Math.sin(i * 0.6) * 2 + Math.random() * 1).toFixed(1))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof PhysicalExamRevenueRatioData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    personalExamRevenueRatio: {
      value: lastData.personalExamRevenueRatio,
      change: prevData ? (lastData.personalExamRevenueRatio - prevData.personalExamRevenueRatio) : 0
    }
  };
};
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">体检收入占比分析</h1>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.data[indicator.key as keyof PhysicalExamRevenueRatioData].toFixed(1)}%
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>个人体检收入占比反映体检业务在医院整体收入中的重要性</li>
              <li>计算公式：个人体检收入/门急诊收入(含体检收入)×100%</li>
              <li>支持按天、月、季度、年查看不同时间粒度的数据趋势</li>
              <li>同比环比分析帮助了解体检收入占比的发展趋势和季节性变化</li>
              <li>点击指标标签可控制图表中对应数据线的显示/隐藏</li>
              <li>该指标有助于评估体检业务的战略地位和发展潜力</li>
              <li>占比提升通常表示体检业务增长快于其他门急诊业务</li>
            </ul>
          </div>
        </div>
      </section>
