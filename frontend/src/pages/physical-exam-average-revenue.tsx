    ...
    case 'day':
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            averagePhysicalExamCost: parseFloat((850 + Math.sin(i * 0.3) * 200 + Math.random() * 100).toFixed(0))
          }
        });
      }
      break;
    ...
    case 'month':
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            averagePhysicalExamCost: parseFloat((880 + Math.sin(i * 0.5) * 150 + Math.random() * 80).toFixed(0))
          }
        });
      }
      break;
    ...
    case 'quarter':
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            averagePhysicalExamCost: parseFloat((900 + Math.sin(i * 0.7) * 120 + Math.random() * 60).toFixed(0))
          }
        });
      }
      break;
    ...
    case 'year':
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        data.push({
          date: dateStr,
          data: {
            averagePhysicalExamCost: parseFloat((920 + Math.sin(i * 0.6) * 100 + Math.random() * 40).toFixed(0))
          }
        });
      }
      break;
...
const generateMockComparisonData = (chartData: ChartData[]) => {
  ...
  indicators.forEach(indicator => {
    const currentValue = chartData[chartData.length - 1].data[indicator.key as keyof PhysicalExamAverageRevenueData];
...
const calculateStats = (data: ChartData[]) => {
  ...
  return {
    averagePhysicalExamCost: {
      value: lastData.averagePhysicalExamCost,
      change: prevData ? (lastData.averagePhysicalExamCost - prevData.averagePhysicalExamCost) : 0
    }
  };
};
      ...
      <header className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-2xl font-bold text-gray-900">体检均次收入分析</h1>
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {indicators.map((indicator) => (
                      <td key={indicator.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatCurrency(item.data[indicator.key as keyof PhysicalExamAverageRevenueData])}{indicator.unit}
                      </td>
                    ))}
                  </tr>
                ))
        ...
        <div className="flex items-start">
          ...
          <div className="text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-2 text-sm">
              ...
              <li>次均体检费用反映体检服务的平均收入水平</li>
              <li>计算公式：个人体检收入/体检人次数</li>
